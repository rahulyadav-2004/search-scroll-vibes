...
const LoginForm = () => {
  ...
  const handleInputChange = (e) => {
    if (activeTab === 'phone') {
      ...
      setIsValid(/^\S+@\S+\.\S+$/.test(e.target.value) || e.target.value === '');
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (isValid) {
      console.log(activeTab === 'phone' ? `Phone: ${phoneNumber}` : `Email: ${email}`);
    }
  };
  ...
  return (
    <div className="w-full max-w-[370px] bg-white rounded-[20px] shadow-inset-subtle p-8 animate-fade-up">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {activeTab === 'phone' ? (
          <div className="flex items-center gap-2 mb-3 rounded-lg border border-scroll-gray-300 bg-scroll-gray-100 bg-opacity-53 p-2">
            ...
            <Input
              ...
              className="border-none bg-transparent h-auto p-0 text-black"
            />
          </div>
        ) : (
          <div className="mb-3">
            <Input
              ...
              className="rounded-lg border border-scroll-gray-300 bg-scroll-gray-100 bg-opacity-53 p-2"
            />
          </div>
        )}
        
        {!isValid && activeTab === 'email' && email !== '' && (
          <div className="text-scroll-red text-xs mb-2">Please enter a valid email address</div>
        )}
        
        <div className="text-scroll-red text-xs mb-3 cursor-pointer">Forget account or password?</div>
        
        <button type="submit" className="button-primary mb-2" disabled={!isValid}>Sign in</button>
        <button type="button" className="button-secondary mb-3">Sign up</button>
        
        <div className="auth-divider">
          <span className="text-scroll-gray-500 text-xs font-semibold">or use</span>
        </div>
        
        <button type="button" className="button-social bg-white mb-2 hover:bg-scroll-gray-100 active:bg-scroll-gray-200">
        ...
        <button type="button" className="button-social bg-black text-white mb-6 hover:bg-black/90 active:bg-black/80">
